...
Use `withRLS(userId, fn)` from `packages/database/src/rls.ts`:

```ts
import {withRLS} from "@openhospi/database/rls";

const rooms = await withRLS(userId, (tx) =>
    tx.select().from(rooms).where(/* ... */)
);
```
...
- Validators: `createInsertSchema(table)` from `drizzle-orm/zod`
- Connection: Lazy proxy in `packages/database/src/db.ts` (defers until first use)

## Common Gotchas

1. **`numeric` columns return strings** — convert with `Number(value)`
2. **`auth` schema is managed by Supabase** — never modify it
3. **`prepare: false`** is required in the postgres.js driver config
