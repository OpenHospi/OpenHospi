...
async function main() {
  ...
  await db
    .update(rooms)
    .set({
      availableUntil: sql`(${rooms.availableFrom}::date + interval '5 months')::date`,
    })
...
async function downloadImage(imgSeed: string, width: number, height: number): Promise<Buffer> {
  const url = `https://picsum.photos/seed/${imgSeed}/${width}/${height}.jpg`;
...
async function seedProfilePhoto(
  ...
  await db.insert(profilePhotos).values({ userId, slot, url: path }).onConflictDoNothing();

  return path;
}

async function seedRoomPhoto(
  db: ReturnType<typeof drizzle>,
  roomId: string,
  slot: number,
): Promise<void> {
  ...
  await db.insert(roomPhotos).values({ roomId, slot, url: path }).onConflictDoNothing();
}
...
async function seedPhotos(db: ReturnType<typeof drizzle>) {
  ...
  for (let i = 0; i < allProfiles.length; i++) {
    ...
    const paths = await Promise.all(
      Array.from({ length: count }, (_, slot) => seedProfilePhoto(db, userId, slot + 1)),
    );

    // Set avatarUrl to slot-1 path (matches app behavior)
    await db.update(profiles).set({ avatarUrl: paths[0] }).where(eq(profiles.id, userId));
  ...
  for (let i = 0; i < allRooms.length; i++) {
    ...
    await Promise.all(
      Array.from({ length: count }, (_, slot) => seedRoomPhoto(db, roomId, slot + 1)),
    );
