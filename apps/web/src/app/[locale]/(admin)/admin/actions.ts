...
export async function getAggregateStats(): Promise<AggregateStats> {
  ...
  const [
    ...
    db.select({ count: count() }).from(session).where(gte(session.updatedAt, sevenDaysAgo)),
    db.select({ count: count() }).from(session).where(gte(session.updatedAt, thirtyDaysAgo)),
    db.select({ count: count() }).from(user).where(gte(user.createdAt, sevenDaysAgo)),
    db.select({ count: count() }).from(rooms).where(eq(rooms.status, RoomStatus.active)),
    db.select({ count: count() }).from(reports).where(eq(reports.status, ReportStatus.pending)),
    db
...
export async function getUserDetail(userId: string): Promise<UserDetail | null> {
  ...
  const [row] = await db
    ...
    .leftJoin(profilePhotos, and(eq(profilePhotos.userId, profiles.id), eq(profilePhotos.slot, 1)))
